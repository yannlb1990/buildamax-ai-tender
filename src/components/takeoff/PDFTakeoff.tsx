...
diff --git a/src/components/takeoff/PDFTakeoff.tsx b/src/components/takeoff/PDFTakeoff.tsx
index 6ff85a41d86b2be2b066ce3e250d0d188212d3b5..d37bfaff9c481e4f3221a34c2c11dacab12e1a82 100644
--- a/src/components/takeoff/PDFTakeoff.tsx
+++ b/src/components/takeoff/PDFTakeoff.tsx
@@ -1,98 +1,129 @@
...
@@ -177,120 +208,193 @@ export const PDFTakeoff = ({ projectId, estimateId, onAddCostItems }: PDFTakeoff
...
+                    <Select value={String(pageFilter)} onValueChange={(val) => setPageFilter(val === 'all' ? 'all' : Number(val))}>
...
+                      className="flex-1"
+                      size="sm"
+                      onClick={() => downloadFile('measurements.csv', exportMeasurementsToCSV(filteredMeasurements), 'text/csv')}
...
+                      className="flex-1"
+                      size="sm"
+                      onClick={() => downloadFile('measurements.json', exportMeasurementsToJSON(filteredMeasurements), 'application/json')}
+                      disabled={!filteredMeasurements.length}
+                    >
+                      <Download className="h-4 w-4 mr-2" />
+                      JSON
...
+                          <Input
+                            value={m.label}
+                            onChange={(e) => dispatch({ type: 'UPDATE_MEASUREMENT', payload: { id: m.id, updates: { label: e.target.value } } })}
+                            placeholder="Label"
+                            className="h-8"
+                          />
...
+                            <Select
+                              value={m.unit}
+                              onValueChange={(unit: MeasurementUnit) => dispatch({ type: 'UPDATE_MEASUREMENT', payload: { id: m.id, updates: { unit } } })}
+                            >
...
+                              <span className="inline-flex h-3 w-3 rounded-full" style={{ backgroundColor: m.color }} />
+                              <Button
+                                variant="ghost"
+                                size="icon"
+                                className="h-8 w-8"
...
+                        <span>Count: {totalsByUnit.count.toFixed(2)}</span>
                       </div>
                     </div>
                   )}
-                </div>
